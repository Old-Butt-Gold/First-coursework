...
Type
  ...
  protected
      ...
      Function CheckMate(Color: TChessColor; ArrOfPieces: TArrFixedPiecesPos): Boolean;
...
Begin
    ...
    Begin
        I := 1;
        While IsDraw and (I < Length(FWhitePieces) + 1) do
        Begin
            If Not(FWhitePieces[I].IsKilled) Then
            Begin
                X := FWhitePieces[I].IndexI;
                Y := FWhitePieces[I].IndexJ;
                Arr := GetPossibleMoves(X, Y);
                J := Low(Arr);
                While IsDraw and (J < Length(Arr)) do
                Begin
                    IsDraw := CheckForShahItself(Color, X, Y, Arr[J].X, Arr[J].Y);
                    Inc(J);
                End;
            End;
            Inc(I);
    ...
    Begin
        I := 1;
        While IsDraw and (I < Length(FBlackPieces) + 1) do
        Begin
            If Not(FBlackPieces[I].IsKilled) Then
            Begin
                X := FBlackPieces[I].IndexI;
                Y := FBlackPieces[I].IndexJ;
                Arr := GetPossibleMoves(X, Y);
                J := Low(Arr);
                While IsDraw and (J < Length(Arr)) do
                Begin
...
Function TChess.CheckMate(Color: TChessColor; ArrOfPieces: TArrFixedPiecesPos): Boolean;
Var
    X, Y, I, J: Integer;
